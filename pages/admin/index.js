  ...
  const handleStatus = async (id) => {
    ...
    try {
      const res = await axios.put('http://localhost:3000/api/orders/' + id, {
        status: currentStatus <= 2 ? currentStatus + 1 : '',
      });
          ...
          <tbody>
            {productList.map((product) => (
              <tr className={styles.trTitle} key={product._id}>
                <td>
                  <Image src={product.img} alt={product.title} width={50} height={50} className={styles.imgContainer} />
                </td>
                <td title={product._id}>{product._id.slice(0, 5)}...</td>
                <td>{product.title}</td>
                <td>₹{product.prices.map((price) => price).join(' ₹')}</td>
                <td>
          ...
          <tbody>
            {orderList.map((order) => (
              <tr className={styles.trTitle} key={order._id}>
                <td title={order._id}>{order._id.slice(0, 5)}...</td>
